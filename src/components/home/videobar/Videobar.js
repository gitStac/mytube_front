...
function Videobar() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            await axios.get(`${process.env.REACT_APP_secret_backEnd}/video/video_get`)
                .then((res) => {
                                        ...
                                        <Link to={`/video/${v._id}`} state={v}>
                                            ...
                                            <div className='videobar_videoInfoDiv'>
                                                <div className='videobar_imageDiv'>
                                                    <img className='bg-light' src={v.channel_id.image} alt='img' />
                                                </div>
                                                <div className='videobar_contentDiv'>
                                                    <div className='videobar_vDesc w_color'>
                                                        {v.title}
                                                    </div>
                                                    <div className='m-0 font_style w_mute'>{v.channel_id.name}</div>
                                                    <div className='m-0 font_style w_mute'>
                                                        <span>{v.views} views</span>
                                                        <span className='m-3'>{moment(v.createdAt).fromNow()}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </Link>
