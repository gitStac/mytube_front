...
function Topbar() {
    ...
    return (
        <Navbar sticky="top" className='py-3 total_back' expand="lg">
            <Container>
                <Navbar.Brand><i className="fa-brands fa-youtube nav_brandlogo"></i>
                    <Link to='/'> <span className='w_color'>myTube</span></Link>
                </Navbar.Brand>
                    ...
                    <Nav>
                        <div className='border border-dark rounded-5 px-3 navbar_inputDiv faint_back'>
                            <form onSubmit={searchSubmit}>
                                <input className='faint_back w_color' type='text' onChange={(e) => setSearch(e.target.value)} />
                                <button type='submit'><i className="fa-solid fa-magnifying-glass w_color"></i></button>
                            </form>
                        </div>
                            ...
                            {userData.isLogin ?
                                <>
                                    ...
                                    <Dropdown className='mx-2'>
                                        <Dropdown.Toggle id="dropdown-basic" className='rounded-circle btn-danger'>
                                            <i className="fa-solid fa-user"></i>
                                        </Dropdown.Toggle>

                                        <Dropdown.Menu>
                                            <div className='pb-2'><Link to="/account">Account</Link></div>
                                            <div className='pb-2'><Link to='channel'>Channels</Link></div>
                                            <div className='pb-2'><Link to='subscription'>Subscription</Link></div>
                                            <div onClick={logout} style={{ cursor: 'pointer' }}>LogOut</div>
                                        </Dropdown.Menu>
                                    </Dropdown>
                                </>
                                :
                                <Link to='/signin' className='border border-primary rounded-5 text-primary py-2 px-4 mx-2'>
